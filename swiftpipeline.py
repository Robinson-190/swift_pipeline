...
def run_xrtpipeline_old(
    ...
    if obs_details_dict["OBS_ID"] != obs_id:
        print(
            f"WARNING: the observation ID given does not correspond to the\
                PAT ({obs_details_dict['OBS_ID']},{obs_id})"
        )
    ...
    pipeline_command = f"xrtpipeline indir={raw_data_folder} steminputs=sw{obs_id} \
outdir={output_folder} srcra={ra_decimal} srcdec={dec_decimal} \
cleanup={cleanup} clobber={clobber} createexpomap={createexpomap} > \
{output_folder}log_{obs_id} chatter=5"

    command_list.append(pipeline_command)
